...
@pytest.fixture
def app():
    """This test suite uses `pytest-flask`. `pytest-flask` comes with an out-
    of-the-box test `client` fixture (i.e., an instance of
    flask.Flask.test_client). The client fixture is nifty: it pushes the
    request context to tests, so you can access context-bound objects without a
    context manager.

    However, the pytest-flask `client` fixture expects an `app` fixture
    – provided here.
    """
    return testing_app
...
def pathways_programs(client):
    ...
    Note! The fixture does not `commit` the programs to the database.
    Instead, the SQLAlchemy session holds the objects in-memory, and at
    the end of each test, the session closes and discards objects.
    """
    json_ld = {
